...
export function IngredientSection() {
  return (
    <SectionContainer variant="alt" id="ingredients">
      <div className="flex flex-col lg:flex-row items-start gap-20">
        ...
        <div className="lg:w-2/3 grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
          {INGREDIENTS.map((ing, i) => (
            ...
            >
              <div>
                <h3 className="text-2xl font-black tracking-tight group-hover:text-accent-foreground transition-colors">{ing.name}</h3>
                <p className="mt-4 text-muted/50 group-hover:text-accent-foreground/70 transition-colors">
                   {ing.detail}
                </p>
              </div>
              ...
              <div className="mt-8 flex items-center space-x-2 overflow-hidden">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity duration-700">Essential</span>
              </div>
            </motion.div>
          ))}
        </div>
